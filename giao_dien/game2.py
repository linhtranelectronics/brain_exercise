...
import random
import math
# Không cần import sys nữa

# Khởi tạo Pygame (Sẽ được gọi duy nhất 1 lần trong start_game_2)
# pygame.init() # Bỏ khỏi đây
...
# Khởi tạo các biến Pygame global
screen = None
clock = None
font_large = None
font_medium = None
...
class PlayerCar(pygame.sprite.Sprite):
    ...
    def update(self):
        global clock # Cần clock để lấy thời gian
...
def draw_hud(screen, health, score, target):
    global font_large, font_medium, MAX_HEALTH
    # Health
...
def draw_menu_screen(screen):
    global font_large, font_medium, SCREEN_WIDTH, SCREEN_HEIGHT
...
# Draw Focus Tips Screen
def draw_focus_driving_tips(screen):
    global font_large, font_medium, font_small, SCREEN_WIDTH, SCREEN_HEIGHT
...
def game_loop():
    """Vòng lặp Pygame chính. Trả về True nếu muốn thoát Pygame hoàn toàn, False nếu muốn tiếp tục."""
    global GAME_SPEED, clock, screen, font_large, font_medium, font_small
    ...
    def reset_game():
        global GAME_SPEED
        nonlocal player, all_sprites, road_objects, score, current_target, last_spawn_time, last_target_change
        player = PlayerCar()
        all_sprites = pygame.sprite.Group()
        road_objects = pygame.sprite.Group()
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                # Thoát vòng lặp Pygame và trả về True để báo cho hàm cha đóng Pygame
                return True

            if event.type == pygame.KEYDOWN:
                # --- XỬ LÝ CHUYỂN LÀN ---
                if game_state == GameState.PLAYING and player:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                ...
                if game_state == GameState.MENU:
                    ...
                    if back_rect.collidepoint(mouse_pos):
                        ...
                        # Thoát vòng lặp Pygame và trả về True
                        return True
                    ...
                    if resume_rect.collidepoint(mouse_pos):
                        ...
                        game_state = GameState.MENU
                        # Đảm bảo reset game state nếu quay lại menu từ pause
                        player = None
                        all_sprites = pygame.sprite.Group()
                        road_objects = pygame.sprite.Group()
                    ...
                    if menu_rect.collidepoint(mouse_pos):
                        game_state = GameState.MENU
                        # Đảm bảo reset game state nếu quay lại menu từ game over
                        player = None
                        all_sprites = pygame.sprite.Group()
                        road_objects = pygame.sprite.Group()
        ...
        if game_state == GameState.PLAYING:
            ...
            if current_time - last_spawn_time > adjusted_spawn_interval:
                new_object = create_random_object(player.rect if player else None)
        ...
        if game_state in [GameState.PLAYING, GameState.PAUSED, GameState.GAME_OVER]:
            ...
            # Chỉ vẽ HUD nếu player đã được khởi tạo
            if player:
    ...
    # Nếu thoát vòng lặp (do running=False), trả về False để không đóng Pygame
    return False

def start_game_2():
    """Hàm wrapper chính để khởi chạy và quản lý vòng lặp Pygame."""
    global screen, clock, font_large, font_medium, font_small

    # Khởi tạo các biến Pygame khi hàm được gọi
    pygame.init() # Khởi tạo Pygame
    
    # Thiết lập các biến global Pygame
    ...
    # Chạy vòng lặp game
    should_quit = game_loop()

    # Dọn dẹp Pygame trước khi trả lại quyền điều khiển cho main.py
    pygame.quit()
